 ...
 * is one level up. In this repo the layout is identical (npm/install.js next to
 * the root manifest), so the first candidate is the one that resolves. The
 * second is kept for checkouts that nest the installer deeper. Look in both so
 * the installer can be required (and tested) straight from the repo.
 */
